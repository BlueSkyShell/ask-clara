...
- FINDING: LLAMA_TOOL_CALLING_1B_INST_Q4_K (tool-specialized 1B) does NOT emit tool calls through the same QVAC interface Qwen3-1.7B uses successfully — on "send 0.001 ETH to alice" it replies "I'm sorry, I can't assist" with toolCalls:[]. Genuine capability finding (pipeline verified via primary), reported honestly with a chat-template caveat.
