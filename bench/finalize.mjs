...
const cp = R.construct?.primary?.summary;
const ct = R.construct?.toolSpecialist?.summary;
...
const block = [
  ...
  '',
  `**Incorrect actions (built a transaction it shouldn't): ${cp.incorrectActions} of ${R.construct.primary.cases.length} (${pct(cp.incorrectActionRate)}).** ` +
